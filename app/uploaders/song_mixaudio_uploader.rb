class SongMixaudioUploader < CarrierWave::Uploader::Base
  ...
  def encode_audio(format)
    ...
    Rails.logger.info "SongMixaudioUploader::encode_audio(#{format}) starting, tmpfile=#{tmpfile}"
  ...
  def generate_waveform
    directory = File.dirname(current_path)
    Rails.logger.info "SongMixaudioUploader::generate_waveform starting, directory=#{directory}"
    ...
    begin
      ...
      self.model.waveform = File.open(interpolations[:output])
