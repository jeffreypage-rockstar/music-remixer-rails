...
WaveSurfer.create = function(a) {
    ...
    createMasterGain: function() {
        // 8Stem hack: add a compressor/limiter to summed sound
        if (_masterGain == null) {
            _masterGain = this.ac.createGain();
            _masterLimiter = this.ac.createDynamicsCompressor();
            _masterLimiter.threshold.value = -15.0; // this is the pitfall, leave some headroom
            _masterLimiter.knee.value = 12; // brute force
            _masterLimiter.ratio.value = 12.0; // max compression
            _masterLimiter.reduction.value = -1.0; // max compression
