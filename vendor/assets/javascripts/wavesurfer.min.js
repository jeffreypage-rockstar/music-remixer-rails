...
var _masterLimiter = null;
var _masterCompressor = null;
...
var WaveSurfer = {
    ...
    seekTo: function(a) {
        ...
        b && (this.params.scrollParent = !1), this.backend.seekTo(a * this.getDuration()), this.drawer.progress(this.backend.getPlayedPercents()), b || (this.backend.pause(), this.backend.play()), this.params.scrollParent = c, this.fireEvent("seek", this, a)
    },
    stop: function() {
...
WaveSurfer.create = function(a) {
    ...
    init: function(a) {
        this.params = a, this.ac = a.audioContext || this.getAudioContext(), this.lastPlay = this.ac.currentTime, this.startPosition = 0, this.scheduledPause = null, this.states = [Object.create(WaveSurfer.WebAudio.state.playing), Object.create(WaveSurfer.WebAudio.state.paused), Object.create(WaveSurfer.WebAudio.state.finished)], this.createMasterGain(), this.createVolumeNode(), this.createScriptNode(), this.createAnalyserNode(), this.setState(this.PAUSED_STATE), this.setPlaybackRate(this.params.audioRate)
    },
    createMasterGain: function() {
        if (_masterGain == null) {
            console.log("creating master gain")
            _masterGain = this.ac.createGain();

            // Create a compressor node
            _masterCompressor = this.ac.createDynamicsCompressor();
            _masterCompressor.threshold.value = -5.25;
            _masterCompressor.knee.value = 6;
            _masterCompressor.ratio.value = 1.5;
            _masterCompressor.reduction.value = -2;
            _masterCompressor.attack.value = 0;
            _masterCompressor.release.value = 0.25;//time (in seconds) to increase the gain by 10dB

            // Create a limiter node
            _masterLimiter = this.ac.createDynamicsCompressor();
            _masterLimiter.threshold.value = -15; // this is the pitfall, leave some headroom
            _masterLimiter.knee.value = -0.3; // brute force
            _masterLimiter.ratio.value = 12.0; // max compression
            _masterLimiter.attack.value = 0.005; // 5ms attack
            _masterLimiter.release.value = 0.050; // 50ms release

            //_masterGain.connect(_masterCompressor);
            _masterCompressor.connect(_masterLimiter);
            _masterLimiter.connect(_masterGain);
            _masterGain.connect(this.ac.destination);
        }
    },
    ...
    createAnalyserNode: function() {
        // console.log("creating analyser node, connecting to gainNode");
        this.analyser = this.ac.createAnalyser(), this.analyser.connect(this.gainNode)
    },
    createVolumeNode: function() {
        // console.log("creating volume/gain node, connecting to destination");
        if (_masterGain == null) debugger;
        this.ac.createGain ? this.gainNode = this.ac.createGain() : this.gainNode = this.ac.createGainNode(), this.gainNode.connect(_masterCompressor), console.log("mainter gain inputs: " + _masterCompressor.numberOfInputs)
    },
    setVolume: function(a) {
